...
### Pilot Avatars
Pilot avatars are obtained from the `/server/static/user/avatars/` folder. The file name should be the pilot's callsign in lowercase where spaces has been replaced for underscores (_) and in the .png file type. The file should be ~~a square image, preferably 256x256 or 512x612 pixels~~. If no avatar is found it will show the default avatar.
